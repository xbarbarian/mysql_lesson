...
CREATE TABLE users (
  ...
  roles_id INT UNSIGNED NOT NULL COMMENT "Cсылка на роль пользвателя", 
...
CREATE TABLE profiles (
  ...
  country id INT UNSIGNED COMMENT "ссылка на страну проживания",
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT "Время создания строки",  
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT "Время обновления строки"
) COMMENT "Профили"; 
...
CREATE TABLE courses (
  ...
  courses_type_id INT UNSIGNED NOT NULL COMMENT "ссылка на тип курса"
) COMMENT "Курсы"  
...
CREATE TABLE posts (
  ...
  users_id INT UNSIGNED NOT NULL COMMENT "ссылка на пользвателя",
...
study_group_id INT UNSIGNED NOT NULL COMMENT COMMENT 'ID группы',
user_id INT UNSIGNED NOT NULL COMMENT 'связь с пользвателем',
created_at datetime DEFAULT current_timestamp() COMMENT 'Дата добавления медиа файла',
PRIMARY KEY (study_group_id, users_id) COMMENT 'Ключ по двум полям'
) COMMENT 'Связь групп учащихся и пользователей';

-- Таблица связей групп учащихся и курсов
CREATE TABLE list_courses_goup (
study_group_id INT UNSIGNED NOT NULL COMMENT COMMENT 'ID группы',
courses_id INT UNSIGNED NOT NULL COMMENT 'связь с курсом',
created_at datetime DEFAULT current_timestamp() COMMENT 'Дата добавления медиа файла',
PRIMARY KEY (study_group_id, courses_id) COMMENT 'Ключ по двум полям'
) COMMENT 'Связь групп учащихся и курсов';

-- Таблица связей пользователей и курсов
CREATE TABLE list_courses_users (
users_id INT UNSIGNED NOT NULL COMMENT COMMENT 'ID пользвателя',
courses_id INT UNSIGNED NOT NULL COMMENT 'связь с курсом',
created_at datetime DEFAULT current_timestamp() COMMENT 'Дата добавления медиа файла',
PRIMARY KEY (users_id, courses_id) COMMENT 'Ключ по двум полям'
